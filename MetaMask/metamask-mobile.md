...
- **[ts-NEW] JSDoc Multi-Line Bullet Pitfall**: The repo's `jsdoc/check-indentation` ESLint rule rejects multi-line bullets inside JSDoc blocks (continuation lines like ` *   continuation` raise "There must be no indentation"). Keep each bullet on a single line, or restructure as separate paragraphs. `void promise.catch(...)` also raises `no-void` — drop the `void` and keep the `.catch()`. Inline `type Foo = {...}` raises `@typescript-eslint/consistent-type-definitions` — use `interface Foo {...}`.
