...
- **[ts-007] Detect react-compiler bailouts with the babel logger, not eslint**: The ESLint `react-compiler/react-compiler` rule only reports a subset (mainly "skipped because a React ESLint rule was disabled"). It does NOT report "Cannot access refs during render", "value blocks within try/catch", or BuildHIR lowering bailouts — those come from the full compiler. To detect/verify, compile the file with `@babel/core` (`presets:['babel-preset-expo']`, `plugins:[['react-compiler',{target:'18',logger}]]`); in the logger, `kind==='CompileError'|'CompileSkip'` is a bailout and `'CompileSuccess'` means optimized. Precise location is at `event.detail.options.{reason,details[].{message,loc.start.line}}`. Use a throwaway script and delete it before committing (don't ship it).
