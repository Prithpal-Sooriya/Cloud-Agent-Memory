...
- **[ts-002] Prefix every `gh issue list` on this repo with the write-issues PAT**: On Cursor cloud VMs the default `gh` token is read-only. `gh issue list --search` returns placeholder `{number:0}` rows; `gh issue list --label` / unfiltered `--state open` can instead return a real empty array `[]` with exit 0. Both look like "no issues" rather than a 403. Always `GH_TOKEN="$CLOUD_AGENT_WRITE_ISSUES_PAT" gh issue list ...` against Prithpal-Sooriya/Cloud-Agent-Memory.
